...
@Stateless
public class ComprobanteRentencionServicios {
	...
	//TODO contructor que se encarga de insertar la cabecera del documento
	public int insertarimpuesto(FacDetRetencion cab_impuesto){
		try {
	...
	public List<FacTiposDocumento> buscartipoDocumento(String codigo) throws Exception{
		Query q;
		try{
			q =  em.createQuery("SELECT E FROM FacTiposDocumento E where E.idDocumento = :codigo") ;
			q.setParameter("codigo",codigo);			
		}catch (Exception e) {
			e.printStackTrace();
			throw new Exception("Error al Buscar el registro");
